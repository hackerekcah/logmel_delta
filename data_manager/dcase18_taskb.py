...
class Dcase18TaskbData:
    ...
    def extract_logmel(self, wav_path):
        ...
        :param wav_path:
        :return: fea of dim (1, 2, frequency, time), first dim is added
        """
        ...
        # drop the last frame
        fea = fea[:, :-1]
        fea_delta = librosa.feature.delta(fea)
        # stack over a new axis, then expand another new axis
        return np.expand_dims(np.stack([fea, fea_delta], axis=0), axis=0)
    ...
    def show_spec_by_name(self, wav_name):
        ...
        with h5py.File(self.dev_h5_path, 'r') as f:
            ...
            plt.figure()
            librosa.display.specshow(spec_data[0][0])
            plt.title(wav_name+'spec')
            librosa.display.specshow(spec_data[0][1])
            plt.title(wav_name+'delta')
            plt.show()
