...
class TaskbStandarizer:
    ...
    def calc_mu_sigma(self, data):
        ...
        :param data: (batch, channel, frequency, time)
        :return: mu, sigma numpy vector
        """
        # transpose to (batch, time, channel, frequency)
    ...
    def load_dev_standrized(self, mode='train', device='a', norm_device='a'):
        ...
        # (batch, channel, frequency, time) transpose to (batch, time, channel, frequency)
        data = np.transpose(data, [0, 3, 1, 2])
        data = (data - mu) / sigma
        # transpose back to (batch, channel, frequency, time)
    ...
    def load_normed_spec_by_name(self, wav_name=None, norm_device=None):
        ...
        # (batch, channel, frequency, time) transpose to (batch, time, channel, frequency)
        spec_data = np.transpose(spec_data, [0, 3, 1, 2])
        spec_data = (spec_data - mu) / sigma
        # transpose back to (batch, frequency, time)
        spec_data = np.transpose(spec_data, [0, 2, 3, 1])
...
if __name__ == '__main__':
    ...
    ori_standarizer.load_dev_standrized(mode='train', device='a', norm_device='a')
